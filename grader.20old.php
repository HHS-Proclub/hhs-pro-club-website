<?php
// This grader was made before JudgeLite (a custom submission judge for challenge problems) was implemented.
// It is still being used for version 1 problems.
...
if ($_SERVER["PHP_SELF"] === "/grader.20old.php") {
	...
	if ($lang === 10) {
		...
		if ($count === 0) {
			...
			showResult("Class name is incorrect.", "Did you declare the class with 'public class ".$prob."'?", true);
			die();
		}
	}
	...
	if ($result['result'] === 11) {
		...
		showResult("Result: Compilation error.", "Try compiling your code locally for more details.", true);
		die();
		...
		showResult("Ideone internal error. Please try again later.", "If the error persists, let an officer know.", true);
		die();
	} elseif ($result['result'] !== 15) {	// status is not success
		storeResult($prob, $user, $lang, false, 'Unknown error.');
		showResult("Result: Unknown error.", "If the error persists, let an officer know.", true);
		die();
	}
...
<form id="grader" action="/grader.20old.php" enctype="multipart/form-data">
...
?>
	...
	document.getElementById("grader").onsubmit = function (e) {
		...
		xhr.open("POST", "/grader.20old.php");
