...
<?php if ( has_post_thumbnail() OR $pm_has_video ) { ?>
		...
		<?php if ( $pm_has_video ) { echo '<span class="pm-icon"><i class="icon-play"></i></span>'; } ?>
			...
			<div class="entry-content">
				...
				$excerpt_count = of_get_option('pm_excerpt_count');	
					...
					if (of_get_option('pm_excerpt') == 'moretag') 
						the_content( __( ' <span class="label label-cat" style="background-color: '. $category_color .'">Continue reading <span class="meta-nav">&rarr;</span></span>', 'powermag' ) );
				
					else {
						 echo '<p>';
						 echo string_limit_words($excerpt,$excerpt_count);
						 echo ' <a href="' . get_permalink() . '" title="' . get_the_title() . '" class="label label-cat" style="background-color: '. $category_color .'">Continue Reading<span class="meta-nav">&rarr;</span></a>';
						 echo '</p>';
					};
				?>
