				...
				if (has_post_thumbnail($post->ID))
				{
					...
					<span class="img-frame">
						<a href="'.get_permalink($post->ID).'">
							<img src="'.$image_url[0].'" width="95" height="53" alt="'. $post->post_title .'" />';
						
						if ($pm_has_video) { $return_html.= '
							<span class="pm-icon"><i class="icon-play"></i></span>'; }
					...
					$return_html.= ' <a href="'.get_permalink($post->ID).'"><span class="img-frame no-thumb"><span class="no-thumb-img">';
						...
						if ($pm_has_video) {$return_html.= '<span class="pm-icon"><i class="icon-play"></i></span>';}
...
add_action( 'widgets_init', 'pm_twitter_load_widgets' );

function pm_twitter_load_widgets() {

	register_widget( 'pm_twitter_widget' );

}
class pm_twitter_widget extends WP_Widget {

	/**
	 * Widget setup.
	 */

	function pm_twitter_widget() {

		/* Widget settings. */
		$widget_ops = array( 'classname' => 'pm_twitter_widget', 'description' => __('Display your latest Twitter feeds', 'powermag') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'pm_twitter_widget' );

		/* Create the widget. */
		$this->WP_Widget( 'pm_twitter_widget', __('PowerMag - Twitter Feeds', 'pm_twitter_widget'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		wp_enqueue_script( 'tweet', get_template_directory_uri() . '/js/jquery.tweet.min.js', array('jquery'), '1.0', true );
		...
		jQuery(function($){
				$(".tweet").tweet({
					...
					join_text: ' <a target="_blank" href="https://twitter.com/<?php echo $username ?>">@<?php echo $username ?></a><br />',
					outro_text: '<span><a target="_blank" class="twtr-join-conv" href="https://twitter.com/<?php echo $username ?>"><?php _e('Join the conversation', 'powermag'); ?></a></span>', 
					avatar_size: 25,
					count: "<?php echo $tweetcount ?>",
					loading_text: '<div class="tweet-loading"></div>',
				});
...
<p>
	<label for="<?php echo $this->get_field_id( 'tweetcount' ); ?>">Tweets Count</label>
