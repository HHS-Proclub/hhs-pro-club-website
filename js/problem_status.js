...
function resetStatusBox() {
    document.getElementById("submission-result-box").className = "";
		document.querySelector("#submission-result-box .progress-box").className = "progress-box";
		document.querySelector("#submission-result-box .progress-box").style.width = "0%";
}
...
function runInterval() {
    ...
    xhr.open("GET", "http://judgelite.westus2.cloudapp.azure.com/api/get_status/" + urlParams.get("job_id"), true);
    xhr.send();
}
...
function handleRequestError() {
    ...
    if (numFailed > 3) {
        statusText.innerHTML = "Error contacting server. Please try again in a bit.";
				document.querySelector("#submission-result-box .loader").style.display = "none";
        return;
    } else {
...
function updateResults(resp) {
    ...
    if (resp["status"] == "queued") {
        ...
        document.querySelector("#submission-result-box").classList.add("submission-result-compile-error");
        document.querySelector("#submission-result-box .test-results-box").innerText = "Error code: " + 
resp["error"];
        return false;
    }
    ...
    if (resp["subtasks"].length == 1 || (resp["subtasks"].length == 2 && resp["is_bonus"][0] == 0 && 
resp["is_bonus"][1] == 1)) {
        ...
        let subtaskIsBonus = resp["is_bonus"][i] == 1;
            ...
            if (test[0] != "--") {
                ...
                if (!subtaskIsBonus || atLeast1Verdict) {
                    displayTestResult(test[0], printSubtasks ? i+1 : 0, printSubtasks ? j+1 : testsTotal+j+1, subtaskIsBonus, 
test[1], test[2].toFixed(1));
                }
            }
        }
